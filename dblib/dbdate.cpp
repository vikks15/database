#include "dbdate.h"
	...
	bool DBDate::CorrectDate()
	{
		...
		if(month!=2 && day>30)
		{
			...
			else if((month<8 && (month%2==0)) || month==9 || month==11)
			{
				//cout<<"Неправильно введена дата. В этом месяце меньше 31ого дня."<<endl;
				return 0;
			}
		}
		else if(month==2 && !IsLeapYear() && day>28) 
		{
			//cout<<"Неправильно введена дата. В этом месяце < 29 дней."<<endl;
			return 0;
		}
	...
	bool DBDate :: IsLeapYear() //leap year check
	{
		...
		//(!year%4 && year%100 || !year%400); wrong
	}
